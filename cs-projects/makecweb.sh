...
function selected_tech() {
    ...
    echo "3) ASP.NET Core Minimal API"
    echo "4) ASP.NET Core WebApi"
    echo "e - To Exit."
    echo -n "Enter: "
}
        ...
        while read -r select; do
            case ${select} in
                ...
                2)  # making webapp using dotnet platform
                    ...
                    exit 0
                ;;
                3)  # making minimal web api using dotnet platform
                    dotnet new web -o "$1"
                    exit 0
                ;;
                4)  # making webapi using dotnet platform
                    dotnet new webapi -o "$1"
                    exit 0
                ;;
                e) exit 1;;
                *)
                    echo "${select} - Invalid option. Use only 1, 2, 3 and 4 OR e to Exit."
                    selected_tech
                    continue
               ;;
            esac
